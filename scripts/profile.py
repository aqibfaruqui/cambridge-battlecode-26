#!/usr/bin/env python3
"""Profile harvester_revamped (or any instrumented bot code) and interpret the
cProfile .pstats files it dumps.

The instrumentation in bots/v10/builders/harvester_revamped.py writes one
.pstats file per Harvester instance (one per subinterpreter) to a profile
directory. This CLI runs a match and aggregates those files into a readable
report.

Usage:
  uv run scripts/profile.py run                    # defaults: v10 vs v10 on separated
  uv run scripts/profile.py run -m default_large1 --bot-b v9
  uv run scripts/profile.py report                 # analyze existing .pstats files
  uv run scripts/profile.py report --sort tottime --top 30
  uv run scripts/profile.py report --filter d_star # only frames matching 'd_star'
  uv run scripts/profile.py callers d_star.py:318  # who calls _succ?
"""
...
def cmd_report(args: argparse.Namespace) -> int:
    s = _aggregate(Path(args.profile_dir))
    s.strip_dirs()
...
def main() -> int:
    ...
    p.add_argument("--profile-dir", default=str(DEFAULT_PROFILE_DIR),
                   help=f"Directory with .pstats files (default: {DEFAULT_PROFILE_DIR})")
    ...
    prep.add_argument("--filter", default=None, help="Regex filter on frame names (e.g. 'd_star' or 'harvester_revamped')")
