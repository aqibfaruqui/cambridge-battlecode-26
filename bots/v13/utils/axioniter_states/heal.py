...
def _claim_target(self: Axioniter, c: Controller, damaged: list[Position]) -> Position | None:
    """
...
def _try_enter_heal(self: Axioniter, c: Controller) -> bool:
    from builders.axioniter import AxioniterState
    ...
    self.state = AxioniterState.HEAL
    return True


def _heal(self: Axioniter, c: Controller) -> None:
    from builders.axioniter import AxioniterState
    ...
    if committed is None and (attacker is None or self.heal_idle_turns >= _IDLE_EXIT_TURNS):
        self.state = self.heal_prev_state if self.heal_prev_state is not None else AxioniterState.SEEK
