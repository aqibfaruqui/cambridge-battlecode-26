...
def _reset_placing_state(self: Harvester) -> None:
...
def _abort_to_seek(self: Harvester) -> None:
    _reset_placing_state(self)
    self.state = type(self.state).SEEK


def _target_still_valid(self: Harvester, c: Controller) -> bool:
...
def _do_ring(self: Harvester, c: Controller, ore_pos: Position) -> None:
...
def _do_step_off(self: Harvester, c: Controller, ore_pos: Position) -> None:
    if self.current_pos != ore_pos:
...
def _do_build(self: Harvester, c: Controller, ore_pos: Position) -> None:
    if self.current_pos == ore_pos:
        self.placing_phase = STEP_OFF
        return
    _finish_build(self, c)
