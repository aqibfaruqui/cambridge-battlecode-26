...
def _explore_lane(self: Axioniter) -> tuple[str, int]:
...
def _seek_dynamic_blockers(self: Axioniter, c: Controller) -> list[tuple[int, int]]:
...
def _pick_explore_target(
    self: Axioniter, lane_axis: str, lane_sign: int
) -> Position | None:
    """
...
def _seek_direction(
    self: Axioniter, c: Controller, move_target: Position
) -> Direction | None:
    w = c.get_map_width()
    h = c.get_map_height()
...
def _seek(self: Axioniter, c: Controller):
